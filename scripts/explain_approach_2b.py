...
label_m_history = label_m.fit(
    data['X'], y_cat, batch_size=20, epochs=200, verbose=0, shuffle=True)
...
print(sent_dec_output.shape)
features_m.compile(
    optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
features_m.fit([x_latent, sent_dec_output, data['x_seq']], data['y_seq'],
               batch_size=20, epochs=50, verbose=1, shuffle=True)
