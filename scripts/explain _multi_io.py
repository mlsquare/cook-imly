...
def paths_model(initialize=True, rnn_cell='gru', latent_dim=5):
    ...
    output_chars = Dense(vocab_size, activation='softmax',
                         name='op_sent')(merge_layer)
...
def features_model(initialize=True, rnn_cell='gru', latent_dim=5):
    ...
    hidden_state_x = Input(shape=(latent_dim,), name='hidden_x')
    input_seq_features = Input(shape=(maxlen_seq, vocab_size), name='ip_seq')
    input_sent_features = Input(shape=(maxlen_seq, 2), name='ip_sent') # Change shape to max_len of new sent
    if rnn_cell == 'gru':
        RNN = GRU
    else:
        RNN = LSTM
    ...
    if initialize:
        ...
        decoder_outputs_1 = decoder_1(input_seq_features)
    ...
    output_chars = Dense(vocab_size, activation='softmax',
                         name='op_sent')(merge_layer)
    model = Model([hidden_state_x, input_seq_features,
...
label_m_history = label_m.fit(
    X, y_cat, batch_size=20, epochs=300, verbose=0, shuffle=True)
...
# features_m.fit([x_latent,x_seq],y_seq,batch_size =20, epochs = 1,verbose=0, shuffle=True)
...
new_path_list = []
# for _, val in enumerate(pred_path_list):
#     new_path_list.append([n for n in val if n != 'S' and n != 'E'])
...
features = []
labels = []
pred_path_list = []  # Reseting to test!

for _, val in enumerate(df.iloc[:, 4]):
    pred_path_list.append(val.split())

for _, val in enumerate(pred_path_list):
    new_path_list.append([n for n in val if n != 'S' and n != 'E'])

paths_lengths = np.array([len(xi) for xi in new_path_list])


print('new_path_list -- ', new_path_list)
print('\nfeat_seq -- ', feat_seq)

print('\nlen_path -- ', len(new_path_list))
print('\nlen_seq -- ', len(feat_seq))
...
x_sent_new = np.zeros((len(sentences), maxlen_seq, 2), dtype=np.bool)

# Populate inputs
print(len(new_path_list))
for i, sentence in enumerate(new_path_list):
    for t, char in enumerate(sentence):
        # print(i, t, char_indices[char])
        x_sent_new[i-1, t-1, char_indices[char]] = 1
        # Edit to i-1, but why?


x_latent = get_hidden_x(x_feat_seq, model=label_m)

print('x_sent -- ', x_sent.shape)
print('x_sent_new -- ', x_sent_new.shape)
print('x_seq -- ', x_seq.shape)
print('x_latent -- ', x_latent.shape)

features_m.compile(
    optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
features_m.fit([x_latent, x_seq, x_sent_new], y_seq, batch_size=20,
               epochs=1, verbose=1, shuffle=True)


# def predict_feats(x, feature_model=features_m, path_model=path_m,
#                   label_model=label_m, latent_dim=latent_dim, feature_size=feature_size):
#     n = x.shape[0]
#     x_f = x.reshape(1, feature_size)
#     token = 'S'
#     feat_tok = feat_seq[0][0] ## Starting feature
#     cont = True
#     text = [token]
#     seq = []
#     feat = []
#     x_sent = np.zeros((1, max_temp, 4), dtype=np.bool)
#     print(x_sent.shape)
#     x_seq = np.zeros((1, maxlen_seq, 4), dtype=np.bool)
#     x_latent = get_hidden_x(x_f, model=label_model)
#     x_latent = x_latent.reshape(1, latent_dim)
#     print(char_indices)
#     char_indices_temp = {'S': 0, 'L': 1, 'R': 2, 'E': 3}
#     indices_char_temp = {0: 'S', 1:'L', 2: 'R', 3: 'E'}
#     x_sent[0, 0, char_indices_temp[token]] = 1
#     x_seq[0, 0, feat_tok] = 1
#     # x_seq[0,0,feat_tok] = 1
#     pred = label_model.predict(x_f)
#     label = [np.argmax(pred[0])]
...
#         char_index = np.argmax(pred[0])
#         x_sent[0, index, char_index] = 1
#         next_char = indices_char_temp[char_index]
#         text.append(next_char)
#         index += 1
#         if next_char == 'E':
#             cont = False
...
#     index = 1
#     x_sent_new = np.zeros((1, maxlen_seq, 2), dtype=np.bool)
#     for i in range(1, len(text)-2):
#         char = text[i]
#         x_sent_new[0, i-1, char_indices[char]] = 1
#         pred = features_m.predict([x_latent, x_seq, x_sent_new])
#         next_feat = np.argmax(pred[0])
#         x_seq[0, index, next_feat] = 1
#         seq.append(next_feat)
#         index += 1
#     return [seq, text]
...
#     curr_feat = np.array([df.iloc[i, 0:4]])
#     # path,label,seq= sample_paths(curr_feat)
#     feature, path = predict_feats(curr_feat)
#     print(feature)
#     # pred_path_list.append(path)
#     # print('actual vs predicted: ', df.iloc[i, 4], ' vs ', ' '.join(
#     #     path), 'labels: ', df.iloc[i, 5], label[0])
...
# # Bivariate time series t1 - path, t2 - seq

# # Issues
# # 1) Will not work when predicted values length is greater or smaller
# # 2) x_sent_2 -- max_len and instances can vary
# # 3) x_sent[i, t, char_indices[char]] = 1 --> IndexError: index 100 is out
# #  of bounds for axis 0 with size 100
